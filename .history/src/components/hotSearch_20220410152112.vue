...
export default ({
  ...
  async mounted() {
    ...
    for(let i=0;i<=parseInt(res.data.result.hots.length/2);i++)
      this.hots1 = res.data.result.hots[i]
    for(let i=parseInt(res.data.result.hots.length/2)+1;i<res.data.result.hots.length-1 ;i++)
      this.hots2 = res.data.result.hots[i]
