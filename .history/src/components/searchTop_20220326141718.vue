<template>
  <div class="searchTop">
    ...
    <!--  musicList  -->
    <div class="list" v-show="searchSongs.length != 0">
      <div class="playItem" v-for="(item, i) in searchSongs" :key="i">
        <div class="list-left" @click="setPlay(item, i)">
...
export default({
  ...
  methods: {
    ...
    ...mapMutations(['setPlayIndex', 'setPlay']),
    historySearch(item) {
    ...
    setPlay(item, i) {
      ...
      let plist = {
        ...
        al: {
          ...
          pic_str: item.al.pic_str,
        }
      }
      console.log([plist]);
      this.$store.commit('setPlaylist', [plist])
